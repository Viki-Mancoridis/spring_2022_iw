...
function loss_neuralode(p, i_exp, is_end_of_epoch_loss)
    ...
    if (is_end_of_epoch_loss)
      #   loss = loss + loss_cos(p)
        loss = loss + (0.1 * loss_collision_limit(p, i_exp))
    end
    return loss
end
...
for epoch in epochs
    ...
    for i_exp in randperm(n_exp_train)
        ...
        total_grad = grad .+ (collision_limit_grad .* 0.1)
      #   total_grad = grad
    ...
    for i_exp in 1:n_exp
        loss_epoch[i_exp] = loss_neuralode(p, i_exp, true)
    end
    ...
    cb(p, loss_train, loss_val, near_the_end); # near_the_end
    i_epoch += 1;
end
