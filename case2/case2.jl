...
for i in 1:n_exp
    ...
    prob_trueode = ODEProblem(trueODEfunc, u0, tspan, k, kwargshandle=KeywordArgSilent) # removed , kwargshandle=KeywordArgSilent
    ode_data = Array(solve(prob_trueode, alg, saveat=tsteps, kwargshandle=KeywordArgSilent))[1:end - 1, :] # removed , kwargshandle=KeywordArgSilent
...
u0 = u0_list[1, :];
prob = ODEProblem(crnn, u0, tspan, saveat=tsteps, atol=atol, rtol=rtol, kwargshandle=KeywordArgSilent) # removed , kwargshandle=KeywordArgSilent
